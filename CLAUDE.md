...
.user.js を変更した場合は、同一の変更（同じコミット）で `README.md` を現状に合わせて更新する。  
`spec.md` が存在する場合は、それも同様に更新する。

- 更新対象となるのは、以下のようにドキュメントの記述と実装が食い違う変更である
    - 動作・挙動の変更、機能の追加・削除
    - 設定項目の追加・削除・既定値の変更
    - `@match` / `@exclude` などによる適用範囲の変更
    - セットアップ手順に影響する変更（`@grant` の増減、外部APIの利用開始など）
- 記述に影響しない変更（リファクタリング、typo修正、フォーマット修正など）では、更新は不要とする
- ドキュメントに書かれていない挙動を変更した場合は、その項目を新たに追記する
